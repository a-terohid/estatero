...
export async function generateMetadata( { params: { userId } }: { params: { userId: string } }): Promise<Metadata> {
  ...
  // Find the agent by ID from the database
  ...
  const fullName = `${agent.name || ''} ${agent?.last_name}`;
  ...
  return {
    ...
    openGraph: {
      title: `${fullName} | Estatero Admin Panel`,
      description: `View and manage the profile of ${fullName} on the Estatero real estate admin dashboard.`,
      url: `https://estatero.vercel.app/dashboard/users/${agent._id}`,
      type: "profile",
      images: [
        {
    ...
    twitter: {
      ...
      title: `${fullName} | Estatero Admin Panel`,
      description: `View and manage the profile of ${fullName} on the Estatero real estate admin dashboard.`,
      images: [agent.profile_picture || "/img/thumbnail.png"],
    },
  };
}
...
const Page = async ({ params }: { params: { userId: string } }) => {
    ...
    const agent = await User.findById(params.userId) || await Agent.findById(params.userId); ;
    ...
    return <Admins_AgentDashboardpage user={agent} />;
};
