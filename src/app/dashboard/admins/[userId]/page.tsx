...
export async function generateMetadata( { params: { userId } }: { params: { userId: string } }): Promise<Metadata> {
  ...
  const user = await User.findById(userId) || await Agent.findById(userId);
...
const Page = async ({ params }: { params: { userId: string } }) => {
    ...
    const Client = await User.findById(params.userId) || await Agent.findById(params.userId); ;
    ...
    return <Admins_AgentDashboardpage user={Client} />;
};
