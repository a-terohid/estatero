        ...
        <div className=''>
           {
            item?.children.length  ?  <div>
                    ...
                    <details className="group">
                        ...
                        <div className='ml-8 my-3'>
                            <ul className='flex flex-col gap-y-3 text-Body-RL-Small scale-up-ver-top'>
            ...
            </div> : <Link className={`p-1 flex items-center gap-x-1  ${pathname === item.href ? "bg-Greyscale-100  text-Greyscale-600 rounded-md px-2 my-1" : ""} `} href={item.href}>{item.icon}{item.name}</Link>  /* Simple link for items without children */
           }
        </div>
    );
};
